...
from ..body_sync import (
    ...
    yeni_ileti_govdesini_ek_indirmeden_kaydet,
)
from ..attachment_cache import ekleri_onbellekten_al
...
def eml_dosyasi_sec(self):
    dlg = wx.FileDialog(
        self,
        _("Daha önce kaydettiğiniz EML dosyasını seçin."),
        wildcard=_("EML dosyaları (*.eml)|*.eml"),
...
def eml_dosyasini_ac(self, event=None):
    ...
    if not str(dosya_yolu).lower().endswith(".eml"):
        ui.message(_("Lütfen EML uzantılı bir dosya seçin."))
        self.liste.SetFocus()
        return
    ...
    cevap = gui.messageBox(
        _("Seçtiğiniz EML dosyası Gelen Kutusuna eklenecektir. "
...
def sunucudan_eml_dosyasini_ac(self, dosya_yolu):
    ...
    try:
        ...
        if not dosya_yolu or not os.path.exists(dosya_yolu):
            raise MailHatasi(_("EML dosyası bulunamadı."))
        if not dosya_yolu.lower().endswith(".eml"):
            raise MailHatasi(_("Lütfen EML uzantılı bir dosya seçin."))
        ...
        with ImapBaglantisi(ayarlar) as imap:
            ...
            if tip != "OK":
                raise MailHatasi(_("EML dosyası Gelen Kutusuna eklenemedi."))

        guvenli_call_after(self, ui.message, _("EML dosyası Gelen Kutusuna eklendi."))
        guvenli_call_after(
            self,
            self.yenilemeyi_gecikmeli_tetikle,
            _("Gelen Kutusu yenileniyor..."),
            "Gelen Kutusu",
            None,
            None,
            False,
        ...
        guvenli_call_after(self, ui.message, _("EML dosyası açılırken bir hata oluştu. Lütfen dosyayı, bağlantınızı ve hesap bilgilerinizi denetleyin."))
    finally:
...
def secili_epostayi_kaydet(self, event=None):
    ...
    if not mail_id:
        if getattr(self, "liste_modu", LISTE_MODU_EPOSTA) != LISTE_MODU_EPOSTA:
            ui.message(_("Kaydetmek için önce bir klasöre girin ve e-posta seçin."))
        else:
            ui.message(_("Kaydetmek için e-posta seçin."))
        return
    ...
    if 0 <= indeks < len(self.mailler):
        ...
        konu = guvenli_dosya_adi(eposta.get("konu", _("Konusuz")), _("Konusuz"), 60)
        varsayilan_ad = konu
    ...
    dlg = wx.FileDialog(
        ...
        wildcard=_("EML dosyaları (*.eml)|*.eml|TXT dosyaları (*.txt)|*.txt"),
    ...
    mesaj_soyle_ve_sonra_calistir(
        _('E-posta {0} olarak kaydediliyor.').format(bicim.upper()),
...
def txt_kayit_metni_olustur(self, mesaj, icerik, ekler, kaynak_klasor):
    ...
    kimden = gonderen_basligini_gosterime_hazirla(
        mesaj.get("From", _("Bilinmiyor")), _("Bilinmiyor")
    )
    ...
    konu = konu_gosterimini_duzenle(
        guvenli_coz(mesaj.get("Subject", _("Konusuz"))) or _("Konusuz")
    )
    ...
    satirlar.extend(
        [
            f"Konu: {konu}",
            f"Tarih: {tarih}",
            _('Klasör: {0}').format(klasor_gorunen_adi(self.secili_kategori)),
            _('IMAP klasörü: {0}').format(kaynak_klasor),
            _('Ek sayısı: {0}').format(len(ek_adlari)),
        ]
    )
    if ek_adlari:
...
def sunucudan_epostayi_kaydet(self, mail_id, kaynak_klasor, hedef_yol, bicim):
    ...
    try:
        if bicim not in ("txt", "eml"):
            raise MailHatasi(_("Desteklenmeyen kaydetme biçimi."))
        ...
        with ImapBaglantisi(ayarlar) as imap:
            ...
            imap_eposta_boyutunu_denetle(
                ...
                mail_id,
                AZAMI_EPOSTA_ISLEME_BOYUTU,
                _("Kaydedilecek e-posta"),
            )
            tip, veri = imap.uid("FETCH", str(mail_id), "(BODY.PEEK[])")
            if tip != "OK":
                raise MailHatasi(_("E-posta sunucudan alınamadı."))
            ham_veri = ham_mesaj_verisi_al(veri)
            if not ham_veri:
                raise MailHatasi(_("E-posta içeriği alınamadı."))
            ham_eposta_boyutunu_denetle(ham_veri, _("Kaydedilecek e-posta"))
            ...
            try:
                ...
                raise MailHatasi(
                    _('E-posta dosyaya yazılamadı: {0}. Seçilen klasör yazma korumalı olabilir, disk dolu olabilir veya güvenlik yazılımı dosyayı kilitlemiş olabilir.').format(os.path.basename(hedef_yol))
                ) from e
        ...
        guvenli_call_after(self, ui.message, _("Kaydetme işlemi sırasında bir hata oluştu. Lütfen dosya izinlerini ve bağlantınızı denetleyin."))
        guvenli_call_after(self, self.liste.SetFocus)

class KaydetmeSonucPenceresi(wx.Dialog):
    def __init__(self, parent, bicim, hedef_yol):
        super().__init__(parent, title=_("E-posta kaydedildi"))
        ...
        mesaj = (
            _('E-postanız {0} formatında kaydedilmiştir.\n\nKlasör: {1}').format(str(bicim).upper(), self.hedef_klasor)
        )
        metin = wx.StaticText(self, label=mesaj)
        duzen.Add(metin, 0, wx.ALL | wx.EXPAND, 10)

        dugmeler = wx.BoxSizer(wx.HORIZONTAL)
        self.klasor_ac_btn = wx.Button(self, wx.ID_OPEN, label=_("Klasörü aç"))
        self.kapat_btn = wx.Button(self, wx.ID_CLOSE, label=_("Kapat"))
    ...
    def klasoru_ac(self, event=None):
        if not self.hedef_klasor or not os.path.isdir(self.hedef_klasor):
            ui.message(_("Kaydedilen klasör bulunamadı."))
            return
        try:
            ...
            ui.message(_("Kaydedilen klasör açılamadı."))
...
def yeni_posta_yaz(self, event=None):
    try:
        ...
        try:
            gui.messageBox(
                _("E-posta yazma penceresi açılamadı."),
                _("E-posta yazma hatası"),
                wx.OK | wx.ICON_ERROR,
                self,
            )
        except Exception as hata:
            hata_kaydet("E-posta yazma hatası penceresi gösterilemedi.", hata)
            ui.message(_("E-posta yazma penceresi açılamadı."))
...
def _yanit_veya_ilet_onbellek_verisi(ayarlar, mail_id, kaynak_klasor, islem):
    ...
    kimden = str(onbellek.get("sender", "") or _("Bilinmiyor"))
    ...
    return {
        ...
        "kimden_tam": gonderen_gosterimini_al(kimden, _("Bilinmiyor")),
        ...
        "konu": guvenli_coz(onbellek.get("subject", "") or _("Konusuz")) or _("Konusuz"),
...
def secili_mesaji_yanitla_veya_ilet(self, islem):
    ...
    if indeks == -1 or indeks >= len(self.mailler):
        ui.message(_("Lütfen işlem yapmak istediğiniz e-postayı seçin."))
        return
    mail_id = self.mailler[indeks].get("id")
    kaynak_klasor = self.aktif_klasor()
    mesaj_soyle_ve_sonra_calistir(
        _("Yanıt hazırlanıyor.") if islem == "yanitla"
        else _("İletilecek e-posta hazırlanıyor."),
        lambda: arka_planda_calistir(
...
def sunucudan_yanit_veya_ilet_hazirla(self, mail_id, kaynak_klasor, islem):
    ...
    try:
        ...
        with ImapBaglantisi(ayarlar) as imap:
            ...
            imap_ok_mu(tip, _("Seçili klasör açılamadı."))
            imap_eposta_boyutunu_denetle(
                imap,
                mail_id,
                AZAMI_EPOSTA_ISLEME_BOYUTU,
                _("Yanıt veya iletme için e-posta"),
            )
            tip, veri = imap.uid("FETCH", str(mail_id), "(BODY.PEEK[])")
            imap_ok_mu(tip, _("E-posta içeriği alınamadı."))
            ham_veri = ham_mesaj_verisi_al(veri)
            if not ham_veri:
                raise MailHatasi(_("E-posta içeriği boş döndü."))
            ham_eposta_boyutunu_denetle(ham_veri, _("Yanıt veya iletme için e-posta"))
        ...
        guvenli_call_after(self, ui.message, _("E-posta hazırlanırken bir hata oluştu."))
...
def mesaj_oku(self, event):
    ...
    mesaj = _("Taslak düzenleniyor.") if self.taslak_klasoru_mu(kaynak_klasor) else _("E-posta görüntüleniyor.")
    mesaj_soyle_ve_sonra_calistir(
        mesaj,
        baslat,
        ad="E-posta açma",
    ...
    try:
        ...
        if eksik:
            with ImapBaglantisi(ayarlar) as imap:
                ...
                imap_ok_mu(tip, _("Konuşmanın bulunduğu klasör açılamadı."))
        ...
        satirlar = konusma_mesajlarini_listele(eposta, kaynak_klasor, thread_id)
        if not satirlar:
            raise MailHatasi(_("Konuşmadaki e-postalar bulunamadı."))
        bolumler, tum_ekler = [], []
        ekler_eksik = False
        for sira, satir in enumerate(satirlar, 1):
            ...
            govde = str(satir.get("plain_text") or _("İleti gövdesi önbelleğe alınamadı."))
            bolumler.append(
                _('{0}. ileti\nKimden: {1}\nKime: {2}\n{3}Tarih: {4}\n{5}\n{6}').format(
                    sira, kimden, kime, bilgi_satiri, tarih, '-' * 50, govde
                )
            )
        ...
        son = satirlar[0]
        son_kimden = str(son.get("sender") or _("Bilinmiyor"))
        ...
        veri = {
            ...
            "ileti_sayisi": len(satirlar),
            "kimden_tam": gonderen_gosterimini_al(son_kimden, _("Bilinmiyor")),
            ...
            "konu": guvenli_coz(son.get("subject", "") or _("Konusuz")) or _("Konusuz"),
        ...
        guvenli_call_after(self, ui.message, _("Konuşma açılırken bir hata oluştu."))

def ekleri_indirmeyi_baslat(self, mesaj_verisi, hedef_klasor):
    """Yalnız kullanıcı Ekleri Kaydet dediğinde çalışan ek indirme işlemi."""
    arka_planda_calistir(_ekleri_sunucudan_indir_ve_kaydet, self, dict(mesaj_verisi), hedef_klasor)


def _ekleri_sunucudan_indir_ve_kaydet(self, mesaj_verisi, hedef_klasor):
    try:
        ayarlar = ayarlari_yukle()
        mail_id = str(mesaj_verisi.get("id", "") or "")
        klasor = str(mesaj_verisi.get("klasor", "") or "")
        if not mail_id or not klasor:
            raise MailHatasi(_("Eklerin indirileceği e-posta bilgisi bulunamadı."))
        with ImapBaglantisi(ayarlar) as imap:
            tip, _veri = imap.select(klasor, readonly=True)
            imap_ok_mu(tip, _("Seçili klasör açılamadı."))
            tip, veri = imap.uid("FETCH", mail_id, "(BODY.PEEK[])")
            imap_ok_mu(tip, _("Ekler sunucudan alınamadı."))
        ham_veri = ham_mesaj_verisi_al(veri)
        if not ham_veri:
            raise MailHatasi(_("Ekler için e-posta içeriği boş döndü."))
        mesaj = email.message_from_bytes(ham_veri, policy=email_policy.default)
        _icerik, ekler, _atlanan = mesaj_metni_ve_ekleri_cikar(mesaj, ayrintili=True)
        if not ekler:
            raise MailHatasi(_("Bu e-postada kaydedilebilir ek dosya bulunamadı."))
        kaydedilen = 0
        for dosya_adi, icerik in ekler:
            if not icerik:
                continue
            hedef_yol = os.path.join(hedef_klasor, guvenli_dosya_adi(dosya_adi, "ek_dosya"))
            with open(benzersiz_yol(hedef_klasor, os.path.basename(hedef_yol)), "wb") as dosya:
                dosya.write(icerik)
            kaydedilen += 1
        if not kaydedilen:
            raise MailHatasi(_("Kaydedilebilir ek dosya bulunamadı."))
        guvenli_call_after(self, ui.message, _('{0} ek dosya kaydedildi. Klasör: {1}').format(kaydedilen, hedef_klasor))
    except MailHatasi as e:
        hata_kaydet(str(e))
        guvenli_call_after(self, ui.message, str(e))
    except Exception as e:
        hata_kaydet("Ek dosyalar indirilemedi.", e)
        guvenli_call_after(self, ui.message, _("Ekler indirilemedi. Lütfen bağlantınızı ve dosya izinlerini denetleyin."))
...
def sunucudan_icerik_indir(self, mail_id, kaynak_klasor, acma_callback=None):
    ...
    try:
        ...
        if onbellek:
            # Ekler artık otomatik önbelleğe alınmaz; metin gövdesi tek başına
            # iletiyi açmak için yeterlidir.
            ...
            onbellek_kullanilabilir = bool(onbellek) and int(onbellek.get("parser_version") or 0) >= BODY_PARSER_VERSION
            if onbellek_kullanilabilir:
                ...
                veri = {
                    ...
                    "has_attachments": bool(onbellek.get("has_attachments")),
                    "ekleri_indirici": self.ekleri_indirmeyi_baslat,
                    "ekler_eksik": False,
                    "taslak_mi": False,
                }
                if taslak_mi:
        ...
        with ImapBaglantisi(ayarlar) as imap:
            ...
            if tip != "OK":
                raise MailHatasi(_("Seçili klasör açılamadı."))
            if yeni_ileti_govdesini_ek_indirmeden_kaydet(
                imap, ayarlar.get("eposta", ""), mail_id, klasor
            ):
                # Metin gövdesi kaydedildi; aşağıdaki çağrı artık yalnızca
                # yerel önbellekten okur ve ek dosyalarına dokunmaz.
                return self.sunucudan_icerik_indir(
                    mail_id, klasor, acma_callback
                )
            raise MailHatasi(
                _("E-posta metni ek dosyaları indirmeden alınamadı.")
            )
    except MailHatasi as e:
        hata_kaydet(str(e))
        guvenli_call_after(self, ui.message, str(e))
        ...
        guvenli_call_after(self, ui.message, _("E-posta açılırken bir hata oluştu."))
        if callable(acma_callback):
            guvenli_call_after(self, acma_callback, None)
